...
from utils.file_utils import write_json, write_file
import datetime
...
def generate_site_report(domain, outpath="outputs/reports/"):
    ...
    try:
        json_file = f"{outpath}{domain}_report_{timestamp}.json"
        md_file = f"{outpath}{domain}_report_{timestamp}.md"

        write_json(json_file, report_data)
        markdown = f"# Site Report: {domain}\n\n**Generated:** {timestamp}\n\n## Detected Tech\n- " + '\n- '.join(report_data["technologies"]) + "\n\n## OSINT Summary\n"
